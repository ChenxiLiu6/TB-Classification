...
def train(X_tv, y_tv, saved_name, A, A_hat, num):
    ...
    for k in tqdm(range(5)):
        ...
        # save 5 models
        torch.save(models_dict, './MinCutMTPool_saved/MinCutMTPool_Models/mtpool_models' + str(num)+'.pt')
...
def evaluate_test(num,X_test, y_test, X_tv, y_tv, A_test, A_hat_test, A_tv, A_hat_tv):
    ...
    #np.savetxt("MinCutMTPool_saved/MinCutMTPool_ROC/MTMinCutPool.txt", np.column_stack((labels, y_pred_probs)))
...
def main():
    ...
    saved_name = "mtpool_mincut_lr_" + str(args.lr) + "_epochs_" + str(args.epochs)
    ...
    num=5
    # num: 2: add 1 ReLU activation functions in (3) GNN
    # best index: 3 Test Accuracy: 0.618  Sensitivity: 0.612  Specificity: 0.624
    # num: 3: step 4) x = nn.PReLU()(x1) -> x = nn.ReLU()(x1)
    # best index: 1 Test Accuracy: 0.581  Sensitivity: 0.741  Specificity: 0.446
    # num: 4: step 3) gnn and step 4) before flatten() nn.ReLU() -> nn.PReLU()best
    # index: 3 Test Accuracy: 0.586  Sensitivity: 0.576  Specificity: 0.594
    # num 5: epochs=500, patience=50, the rest is the same as num 4
    # best index: 3 Test Accuracy: 0.613  Sensitivity: 0.659  Specificity: 0.574



    #train(X_tv, y_tv, saved_name, A_tv, A_hat_tv, num)

    #------------------------------- 3. Evaluate Test ---------------------------
    best_index = evaluate_test(num, X_test, y_test, X_tv, y_tv, A_test, A_hat_test, A_tv, A_hat_tv)
    history_list = load_history(saved_name, num)
    #plot(history_list)
    #best_index=1
    save_best_history(history_list[best_index], best_index, "MTMinCutPool")
